...
Number.prototype.format = function(f){pattern=(f||'0').toString();if(pattern.indexOf('%')==pattern.length-1)return Number(this*100).format(pattern.substr(0,pattern.length-1))+'%';var sign=this<0?'-':'',val=this<0?-this:this;var str=val.toString();var strInt,strFloat,formatInt=pattern,formatFloat=null;if(/\./g.test(pattern)){formatInt=pattern.split('.')[0];formatFloat=pattern.split('.')[1];}if(/\./g.test(str)){if(formatFloat!=null){var tempFloat=Math.round(parseFloat('0.'+str.split('.')[1])*Math.pow(10,formatFloat.length))/Math.pow(10,formatFloat.length);strInt=(Math.floor(val)+Math.floor(tempFloat)).toString();strFloat=/\./g.test(tempFloat.toString())?tempFloat.toString().split('.')[1]:'0';}else{strInt=Math.round(val).toString();strFloat='0';}}else{strInt=str;strFloat='0';}if(formatInt!=null){var outputInt='',zero=formatInt.match(/0*$/)[0].length;var comma=null;if(/,/g.test(formatInt)){comma=formatInt.match(/,[^,]*/)[0].length-1;}var newReg=new RegExp('(\\d{'+comma+'})','g');if(strInt.length<zero){outputInt=new Array(zero+1).join('0')+strInt;outputInt=outputInt.substr(outputInt.length-zero,zero);}else{outputInt=strInt;}strInt=(outputInt.substr(0,outputInt.length%comma)+outputInt.substring(outputInt.length%comma).replace(newReg,(comma!=null?',':'')+'$1')).replace(/^,/,'');}if(formatFloat!=null){var outputFloat='';var zero=formatFloat.match(/^0*/)[0].length;if(strFloat.length<zero){outputFloat=strFloat+new Array(zero+1).join('0');var outputFloat1=outputFloat.substring(0,zero);var outputFloat2=outputFloat.substring(zero,formatFloat.length);outputFloat=outputFloat1+outputFloat2.replace(/0*$/,'');}else{outputFloat=strFloat.substring(0,formatFloat.length);}strFloat=outputFloat;}else{if(pattern!=''||(pattern==''&&strFloat=='0')){strFloat='';}}return sign+strInt+(strFloat==''?'':'.'+strFloat);};
...
Date.prototype.format = function (f) {f=f||'yyyy-MM-dd';return f.replace('yyyy',this.getFullYear()).replace('MM',this.getMonth()+1).replace('dd',this.getDate()).replace(/(HH|hh)/i,this.getHours()).replace('mm',this.getMinutes()).replace('ss',this.getSeconds()).replace(/(\D)(\d{1})(\D|$)/g,'$10$2$3').replace(/(\D)(\d{1})(\D|$)/g,'$10$2$3');};//两次相同的replace是为了能够匹配2012-3-3 2:9:4这种情况
...
Array.buildAry = function (str,len){var ary = [];for(var i=0;len && i<len;i++){ary.push(str);}return ary;};
...
$.browserName = (function(){var av=navigator.appVersion;if(av.indexOf('Chrome')>0)av=av.replace(/.*Chrome[\D*](\d+).*/g,'Chrome$1');else if(av.indexOf('MSIE')>0)av=av.replace(/.*MSIE[\D*](\d+).*/g,'IE$1');else if(av.indexOf('Firefox')>0)av=av.replace(/.*Firefox[\D*](\d+).*/g,'Firefox$1');else if(av.indexOf('Safari')>0)av=av.replace(/.*Safari[\D*](\d+).*/g,'Safari$1');else if(av.indexOf('rv:11.0) like Gecko')>0)av='IE11';return av;})();
window.trace = function (count) {var caller = arguments.callee.caller;var i = 0;count = count || 10;console.log("***----------------------------------------  ** " + (i + 1));while (caller && i < count) {console.log(caller.toString());caller = caller.caller;i++;console.log("***---------------------------------------- ** " + (i + 1));}};
$(function(){
...
$.extend($.fn.datagrid.defaults,{pageList:[10,20,50,200],rownumbers:true,modal:true,
...
(function($){
	...
	$.openWin.defaults = {divId:'operateWindow',parWin:null,width:800,height:600,collapsible:false,minimizable:false,maximizable:true,closable:true,
			title:'操作窗口',modal:true,url:null,params:{},butParams:[{id:'btn-close',text:'关闭',iconCls:'icon-close'}],onResize:function(){
...
//文件上传组件
(function($){
	$.unsupportBreakpointUpload = history.pushState && window.FormData && window.Blob && window.File && 
			(File.prototype.slice=(File.prototype.slice || File.prototype.webkitSlice));//是否支持断点续传
	$.showFileInWindow = function (opt){//将指定的文件显示在新窗口中
		...
		var opt2 = $.extend({},$.fn.fileupload.defaults,{maxFileSize:10240*1024*1024,maxFileCount:999,titleMaxByte:44},opt);
	...
	$.fn.fileupload = function(opt){//主方法
		...
		th.after('<a iconcls=fa-paperclip>选择文件 </a><a class=history-file>历史上传</a><input type=file class=hidefile /><span class=file-body></span>');
		var btn = th.next(),hisfile=btn.next(),f = hisfile.next(),body=f.next();
		btn.linkbutton();
		...
		hisfile.toggle(!opt.hideHistory).click(function (){
			...
			loadExistFileInfo(opt,th.val(),function(success,data){
				if(success){
					...
					for(var i=0;data&&i<data.length;i++){
						...
						$('.upload-info',span).text('上传成功');
						$('.file-progress-bar',span).width($('.file-progress',span).width());//进度条更新
						$('.file-name',span).html('<a href="'+opt.loadFileUrl+data[i].VC_ID+'" target=_blank>'+data[i].VC_NAME.cutString(opt.titleMaxByte)+'</a>');
					}
				}
				else{
					errinfo.text(data);
		...
		}).trigger('showFileInfo');
		if ($.unsupportBreakpointUpload) {//支持断点续传
			btn.click(function(){//选择文件按钮，使用按钮触发选择文件事件
				f.click();
			});
			f.attr('multiple',opt.multiple);
			...
			hisfile.after('<font color=red>您的浏览器不支持断点续传，请使用谷歌浏览器或其他推荐浏览器</font>');
			var fileCnt = Math.max(1,Math.min(10,opt.maxFileCount));
			btn.click(function(){
				var win = $.openWin({divId:'uploadWin',content:'<div class=upload-file-div><form id="uploadTmpForm" method=post enctype="multipart/form-data"><input type=hidden name=fileType value="'+opt.fileType+'" />'+
					Array.buildAry('<input type=file name=multiFile />',fileCnt).join('')+'</form></div>',
					title:'请选择要上传的附件',width:400,butParams:[{id:'btn-do-upload',text:'上传',iconCls:'icon-up'},{id:'btn-upload-cancel',text:'取消',iconCls:'icon-cancel'}]});
				win.window('resize',{width:400,height:$('.upload-file-div').height()+86}).window('center');
				$('#uploadTmpForm').form({
					url:opt.uploadUrl,
					onSubmit : function() {
						var selFile = false;
						$('.upload-file-div :file').each(function(){if($(this).val()!=''){selFile = true;return false;}});
						if(!selFile){
					...
					success : function(data) {
						$.util.removeLoading();
						returnOptMsgEasyui({data:data,callback:function(successed,result){
							if(successed && result.fileIds){
								th.val(th.val()+(th.val()!=''?',':'')+result.fileIds);
								th.trigger('showFileInfo');//显示已上传的文件信息
							}
						}});
					},
					onLoadError:function(){
						$.util.removeLoading();
						$.messager.alert('提示','文件上传失败!请检查网络是否畅通');
					}
				});
	...
	function uploadFile(filespan,callback){
		...
		filespan.data('filestate',1);//设定为正在上传状态，避免重复上传
	...
	$('head').append('<style>'+
			'.history-file{cursor:pointer;color:#6666CC;margin-left:10px;vertical-align:bottom;line-height:14px;display:inline-block;padding:3px 4px;}'+
			'.history-file:hover{background:#0099CC;color:#fff;}'+
			...
			'.hidefile{position:absolute;left:-9999px;top:0px;}'+
			...
			'.upload-file-div input[type=file]{border:1px solid #ccc;width:95%;margin:5px 5px 0px;;}'+ 
			'</style>');
})(jQuery);